...
def parse_args():
    ...
    # rocFFT options
    parser.add_argument('--gen_pattern', nargs='+', required=False, default=['all'],
                        help='Size patterns to generate (none, pow2, pow3, pow5, pow7, 2D, large, small, all)')
    parser.add_argument('--gen_precision', nargs='+', required=False, default=['all'],
                        help='Precision types generate (single, double, all)')
    parser.add_argument('--gen_groups', type=int, required=False,
                        help='Number of small kernel groups')
    parser.add_argument('--manual_small', nargs='+', required=False,
                        help='Small problem sizes to generate')
    parser.add_argument('--manual_large', nargs='+', required=False,
...
def config_cmd():
    ...
    if args.manual_small:
        cmake_options.append(f"-DGENERATOR_MANUAL_SMALL_SIZE={','.join(args.manual_small)}")
    if args.manual_large:
        cmake_options.append(f"-DGENERATOR_MANUAL_LARGE_SIZE={','.join(args.manual_large)}")
